...
int32_t main(int32_t argc, char **argv) {
    ...
    if ( (0 == commandlineArguments.count("cid")) ) {
        ...
        std::cerr << "[opendlv-device-can-battery] Opening " << CANDEVICE << "... ";
#ifdef __linux__
        ...
        if (socketCAN < 0) {
            ...
            std::cerr << "[opendlv-device-can-battery] Error while creating socket: " << strerror(errno) << std::endl;
        }
        ...
        if (0 != ioctl(socketCAN, SIOCGIFINDEX, &ifr)) {
            ...
            std::cerr << "[opendlv-device-can-battery] Error while getting index for " << CANDEVICE << ": " << strerror(errno) << std::endl;
            return retCode;
        }
        ...
        if (bind(socketCAN, reinterpret_cast<struct sockaddr *>(&address), sizeof(address)) < 0) {
            ...
            std::cerr << "[opendlv-device-can-battery] Error while binding socket: " << strerror(errno) << std::endl;
            return retCode;
        }
        ...
        std::clog << "[opendlv-device-can-battery] Closing " << CANDEVICE << "... ";
        if (socketCAN > -1) {
            close(socketCAN);
        }
